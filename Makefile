...
$(BIN_DARWIN_ARM64): generate/zz_filesystem_generated.go
	...
	go build -o $(TARGET_DIR)/$(BIN_DARWIN_ARM64) -trimpath -ldflags "$(LDFLAGS) -w -s" ./cmd/$(BIN)
...
$(BIN_DARWIN_AMD64): generate/zz_filesystem_generated.go
	...
	go build -o $(TARGET_DIR)/$(BIN_DARWIN_AMD64) -trimpath -ldflags "$(LDFLAGS) -w -s" ./cmd/$(BIN)
...
$(BIN_LINUX_AMD64): generate/zz_filesystem_generated.go
	...
	go build -o $(TARGET_DIR)/$(BIN_LINUX_AMD64) -trimpath -ldflags "$(LDFLAGS) -w -s" ./cmd/$(BIN)
...
$(BIN_LINUX_ARM64): generate/zz_filesystem_generated.go
	...
	go build -o $(TARGET_DIR)/$(BIN_LINUX_ARM64) -trimpath -ldflags "$(LDFLAGS) -w -s" ./cmd/$(BIN)
...
$(BIN_LINUX_PPC64LE): generate/zz_filesystem_generated.go
	...
	go build -o $(TARGET_DIR)/$(BIN_LINUX_PPC64LE) -trimpath -ldflags "$(LDFLAGS) -w -s" ./cmd/$(BIN)

.PHONY: linux-s390x
linux-s390x: $(BIN_LINUX_S390X) ## Build for Linux s390x

$(BIN_LINUX_S390X): generate/zz_filesystem_generated.go
	env CGO_ENABLED=0 GOOS=linux GOARCH=s390x \
	go build -o $(TARGET_DIR)/$(BIN_LINUX_S390X) -trimpath -ldflags "$(LDFLAGS) -w -s" ./cmd/$(BIN)
...
$(BIN_WINDOWS): generate/zz_filesystem_generated.go
	...
	go build -o $(TARGET_DIR)/$(BIN_WINDOWS) -trimpath -ldflags "$(LDFLAGS) -w -s" ./cmd/$(BIN)
